...
namespace Server
{
    ...
    [System.Serializable]
    public class PFConsumableInventoryItemsResponse : AMToolkits.Net.HTTPResponseResult
    {
        public PFResultItemData? Data = null;
    }
    ...
    public partial class PlayFabService
    {
        ...
        /// <param name="user_uid"></param>
        /// <returns></returns>
        public async Task<PFResultItemData?> PFConsumableInventoryItems(string user_uid, string playfab_uid,
                                    List<AMToolkits.Game.GeneralItemData> list,
                                    string reason = "")
        {
            if (_status != AMToolkits.ServiceStatus.Ready)
            ...
            var response = await this.APICall<PFConsumableInventoryItemsResponse>("/internal/services/user/inventory/consumable",
                    new Dictionary<string, object>()
                    {
                        { "user_uid", user_uid },
                        { "playfab_uid", playfab_uid },
            ...
            if (response == null)
            {
                _logger?.LogError($"{TAGName} (User:{user_uid}) ConsumableInventoryItems Failed: ({playfab_uid}) {_client_factory?.LastError?.Message}");
                return null;
            }

            if (response.Data?.Result != AMToolkits.ServiceConstants.VALUE_SUCCESS)
            {
                _logger?.LogError($"{TAGName} (User:{user_uid}) ConsumableInventoryItems Failed: ({playfab_uid}) [{response.Data?.Result}:{response.Data?.Error}]");
                return null;
            }
